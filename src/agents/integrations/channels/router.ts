...
/*
Server side, express (azure app). The router is transport agnostic, so the endpoint only verifies the
signature, answers immediately and hands the payload over.

  import express from "express"
  import { channels } from "<this repo>/lib/agents/integrations/channels"

  const app = express()

  // The signature is an hmac over the EXACT bytes Meta sent, so the raw body has to survive json parsing.
  // Reparsing and re-stringifying changes key order and whitespace and the hmac stops matching.
  app.use("/router", express.json({
    verify: (req, _res, buf) => { (req as any).rawBody = buf },
  }))

  // GET: subscription handshake. Meta calls it once, when the callback url is saved in the app dashboard.
  ...
  // POST: every message of every tenant arrives here.
  app.post("/router", async (req, res) => {

    const raw = (req as any).rawBody
    const ok = await channels.verifySignature(raw, req.get("x-hub-signature-256"), process.env.META_APP_SECRET)
    ...
    // response is already sent. processRouter swallows its own errors, the catch is only a backstop.
    res.sendStatus(200)

    channels.processRouter(req.body).catch(err => console.error("[router] error", err))

  })

Notes:
- One shared Meta App means one META_APP_SECRET and one META_VERIFY_TOKEN, both from env, so the signature
  is verified BEFORE the body is trusted. If a per tenant secret ever appears, only meta-verify.ts and
  this handler change.
...
- An unregistered asset is not an error for the caller: processRouter logs it and returns, the 200 already
  went out.
*/
