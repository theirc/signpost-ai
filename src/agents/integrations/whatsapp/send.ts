...
export async function send({ phone, to, token, message, files = [], quickReplies = [], message_id }: SendParams) {
  ...
  let captionImage: string | undefined

  if (images.length > 0) {
    if (quickReplies.length > 0 && quickReplies.length <= 3) headerImage = images.shift()
    else if (message && quickReplies.length === 0) captionImage = images.shift()
  }
  ...
  if (quickReplies.length > 0) {
    ...
    if (quickReplies.length <= 3) {
      await sendInteractiveReplyButtonsMessage({
        ...
        ...(headerImage && { header: { type: "image", link: headerImage } }),
  ...
  if (message && quickReplies.length === 0) {
    if (captionImage) await sendImageMessage({ phone, token, to, link: captionImage, caption: message })
    else await sendTextMessage({ phone, token, to, body: message })
  }
